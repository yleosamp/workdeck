...
export const userRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get("/profiles/:handle", async (request, reply) => {
    ...
    const normalizedPresence = presence[0] ? {
      ...presence[0],
      sessionStartedAt: utcTimestamp(presence[0].sessionStartedAt),
      updatedAt: utcTimestamp(presence[0].updatedAt) ?? new Date().toISOString()
    } : null;
    return {
      profile: toPublicUser(profile, self),
      friendCount: friendCount[0]?.count ?? 0,
      totals: totals.map((item) => ({ ...item, lastOpenedAt: utcTimestamp(item.lastOpenedAt) })),
      daily,
      presence: canSeePresence ? normalizedPresence : null,
      isFriend: friendship && !self
    };
  });
  ...
  app.get("/p/:handle", async (request, reply) => {
    ...
    const recentlyUsed = recent.map((item) => {
      const timestamp = utcTimestamp(item.lastOpenedAt);
      if (!timestamp) return "";
      const date = new Date(timestamp);
      return `<li><span>${escapeHtml(item.appName)}</span><time datetime="${escapeHtml(timestamp)}">${escapeHtml(date.toISOString().slice(0, 16).replace("T", " "))} UTC</time></li>`;
    }).join("") || "<li><span>Nenhum software usado recentemente</span></li>";
